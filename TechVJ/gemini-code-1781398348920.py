...
async def send_start(client: Client, message: Message):
    ...
    if len(message.text.split()) > 1:
        ...
        if payload.startswith("verify_"):
            # Force subscribe verification for login bypass links too
            ...
            try:
                ...
                if int(p_user_id) == user_id:
                    ...
                    if db_token and db_token == p_token:
                        ...
                        await message.reply_text("❌ **𝚅... 𝙻𝚒𝚗𝚔 𝙴𝚡𝚙𝚒𝚛𝚎𝚍!** Yahi link ek baar use ho chuki hai. Nayi link generate karein.")
                        return
                else:
...
async def save(client: Client, message: Message):
    ...
    # 🛑 PROTECTION 1: Agar user database setting update kar raha hai, toh filter check bypass karo
    ...
    # 🛑 PROTECTION 2: Login System Commands Ko Bilkul Mat Chhedo (Ye pehle bypass ho rhi thi)
    if text.startswith("/") or text.lower() in ["skip", "cancel"]:
        # Is message ko baaki modules/plugins ya dynamic login check ke liye aage jaane do, yahan se return ho jao
        return

    # 1. FORCE SUBSCRIBE GATEKEEPER
    is_joined = await check_force_sub(client, user_id)
    if not is_joined:
        try:
    ...
    # 2. Handle joining via text raw string 
    ...
    # 3. Main Link Processing Section
