...
async def save(client: Client, message: Message):
    ...
    if "https://t.me/" in message.text:
        ...
        # Track videos uploaded to user PM in this specific single/bulk task
...
async def handle_private(client: Client, acc, message: Message, chatid: int, msgid: int, user_uploaded_videos: list):
    ...
    # Get Dump Channel ID from Database
    dump_channel_id = await db.get_dump_id(message.from_user.id)

    if batch_temp.IS_BATCH.get(message.from_user.id): return 
            
    # 🚀 DUMP DELAY FIX FOR ALL FORMATS (PARALLEL UPLOAD EXECUTION)
    tasks = []
    ...
    if "Document" == msg_type:
        ...
        try:
            user_task = client.send_document(chat, file, thumb=ph_path, caption=caption, reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML, progress=progress, progress_args=[message,"up"])
            tasks.append(user_task)
            if dump_channel_id:
                dump_task = client.send_document(int(dump_channel_id), file, thumb=ph_path, caption=caption, parse_mode=enums.ParseMode.HTML)
                tasks.append(dump_task)
            
            results = await asyncio.gather(*tasks, return_exceptions=True)
            if not isinstance(results[0], Exception): user_uploaded_videos.append(results[0])
        except Exception as e:
        ...
        try:
            user_task = client.send_video(chat, file, duration=msg.video.duration, width=msg.video.width, height=msg.video.height, thumb=ph_path, caption=caption, reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML, progress=progress, progress_args=[message,"up"])
            tasks.append(user_task)
            if dump_channel_id:
                dump_task = client.send_video(int(dump_channel_id), file, duration=msg.video.duration, width=msg.video.width, height=msg.video.height, thumb=ph_path, caption=caption, parse_mode=enums.ParseMode.HTML)
                tasks.append(dump_task)
        ...
        try:
            user_task = client.send_animation(chat, file, reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML)
            tasks.append(user_task)
            if dump_channel_id:
                dump_task = client.send_animation(int(dump_channel_id), file, parse_mode=enums.ParseMode.HTML)
                tasks.append(dump_task)
        ...
        try:
            user_task = client.send_sticker(chat, file, reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML)
            tasks.append(user_task)
            if dump_channel_id:
                dump_task = client.send_sticker(int(dump_channel_id), file, parse_mode=enums.ParseMode.HTML)
                tasks.append(dump_task)
        ...
        try:
            user_task = client.send_voice(chat, file, caption=caption, caption_entities=msg.caption_entities, reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML, progress=progress, progress_args=[message,"up"])
            tasks.append(user_task)
            if dump_channel_id:
                dump_task = client.send_voice(int(dump_channel_id), file, caption=caption, caption_entities=msg.caption_entities, parse_mode=enums.ParseMode.HTML)
                tasks.append(dump_task)
        ...
        try:
            user_task = client.send_audio(chat, file, thumb=ph_path, caption=caption, reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML, progress=progress, progress_args=[message,"up"])   
            tasks.append(user_task)
            if dump_channel_id:
                dump_task = client.send_audio(int(dump_channel_id), file, thumb=ph_path, caption=caption, parse_mode=enums.ParseMode.HTML)
                tasks.append(dump_task)
        ...
        try:
            user_task = client.send_photo(chat, file, caption=caption, reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML)
            tasks.append(user_task)
            if dump_channel_id:
                dump_task = client.send_photo(int(dump_channel_id), file, caption=caption, parse_mode=enums.ParseMode.HTML)
                tasks.append(dump_task)
