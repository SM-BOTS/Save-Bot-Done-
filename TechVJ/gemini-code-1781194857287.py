...
async def handle_private(client: Client, acc, message: Message, chatid: int, msgid: int):
    ...
    if db_caption:
        caption = db_caption  # Agar custom caption set hai toh purana bilkul khatam
    else:
    ...
    # Track messages to be automatically deleted
    messages_to_delete = []
    ...
    if "Document" == msg_type:
        ...
        try:
            # 1. User ko video send karo aur list me add karo delete ke liye
            uvsg = await client.send_video(chat, file, duration=msg.video.duration, width=msg.video.width, height=msg.video.height, thumb=ph_path, caption=caption, reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML, progress=progress, progress_args=[message,"up"])
            messages_to_delete.append(uvsg)

            # 2. Agar Dump channel ID hai to video wahan bhi send karo aur delete list me daalo
            if dump_channel_id:
                try:
                    dvsg = await client.send_video(int(dump_channel_id), file, duration=msg.video.duration, width=msg.video.width, height=msg.video.height, thumb=ph_path, caption=caption, parse_mode=enums.ParseMode.HTML)
                    messages_to_delete.append(dvsg)
                except Exception as dump_err:
                    print(f"Dump send failed: {dump_err}")

            # 3. Notification Message trigger (Caption se alag notification)
            notif_text = f"⏳ Yeh file {int(AUTO_DELETE_TIME/60)} minute mein automatic delete ho jayegi!"
            
            unotif = await client.send_message(chat, text=notif_text, reply_to_message_id=uvsg.id)
            messages_to_delete.append(unotif)
            
            if dump_channel_id:
                try:
                    dnotif = await client.send_message(int(dump_channel_id), text=notif_text, reply_to_message_id=dvsg.id)
                    messages_to_delete.append(dnotif)
                except:
                    pass

            # 4. Fire the Auto-delete Worker (Background mein time chalega)
            asyncio.create_task(auto_delete_worker(messages_to_delete, AUTO_DELETE_TIME))
