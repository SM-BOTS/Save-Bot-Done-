...
async def handle_private(client: Client, acc, message: Message, chatid: int, msgid: int, user_uploaded_videos: list):
    ...
    if "Document" == msg_type:
        ...
        try:
            # 🚀 DUMP DELAY FIX: Running parallel uploads using asyncio task gathering
            tasks = []
            
            # Task 1: Upload to user's chat (PM)
            user_task = client.send_video(
                chat_id=chat, 
                video=file, 
            ...
            # Task 2: Upload to personal dump channel (if configured)
            if dump_channel_id:
                dump_task = client.send_video(
            ...
            # Append user message object to track list for auto-deletion later
            uvsg = results[0]
            if not isinstance(uvsg, Exception):
                user_uploaded_videos.append(uvsg)
